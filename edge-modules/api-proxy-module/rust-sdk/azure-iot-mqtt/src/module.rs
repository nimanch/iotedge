...
impl Client {
    ...
    pub fn new(
        ...
        topics: Option<Vec<mqtt3::proto::SubscribeTo>>,
                ...
                State::WaitingForSubscriptions {
                    ...
                    if *reset_session {
                        match std::pin::Pin::new(&mut this.inner).poll_next(cx) {
							...
							std::task::Poll::Ready(Some(Ok(mqtt3::Event::Publication(publication)))) => {
								...
								Ok(InternalMessage::CustomMessage {
										topic_name,
										payload,
									}) => {
...
impl InternalMessage {
    fn parse(publication: mqtt3::ReceivedPublication) -> Result<Self, MessageParseError> {
        ...
        if let Some(captures) = crate::DIRECT_METHOD_REGEX.captures(&topic_name) {
            ...
            match crate::twin_state::InternalTwinStateMessage::parse(publication) {
                ...
                Err(err) => {
                    log::error!(
                        "Message parsing failed with error :{}, defaulting to custom payload",
                        err
                    );
                    Ok(InternalMessage::CustomMessage {
                        topic_name,
